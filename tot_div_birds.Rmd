...
pacman::p_load(devtools, readr, dplyr, tidyr, tibble, purrr, jtools, rsq, broom, HDInterval, cowplot, MCMCglmm, zoo, ggbeeswarm, viridis, gganimate, scatterpie)
...
pred_trans_unscaled <- pred %>% 
  dplyr::mutate_at(vars(Dist, tot_area, SR_GAM, hum), log10) %>% 
  dplyr::mutate(Elev = log10(Elev + 1)) %>% 
  dplyr::mutate(sd_area = log10(sd_area + 1)) %>% 
  dplyr::mutate(res_eff = log10(res_eff + 1)) %>% 
  dplyr::mutate(ext_spp_np = log10(ext_spp_np + 1)) %>% 
  dplyr::mutate(end_spp_np = log10(end_spp_np + 1)) %>% 
  dplyr::mutate_at(vars(no_isl), sqrt) 

pred_trans <- pred_trans_unscaled %>% 
  dplyr::mutate_at(vars(-Archip_10km, -ext_spp_np), scale) %>% 
  dplyr::mutate_at(vars(-Archip_10km, -ext_spp_np), as.numeric)
```
...
# partial r2
rq <- rsq::rsq.partial(m1, type = "sse", adj = FALSE) %>% 
  data.frame() %>% 
  dplyr::bind_cols(data.frame(rsq.partial(m1, type = "sse", adj = TRUE))) %>% 
  dplyr::mutate(partial.rsq_round = round(partial.rsq, digits = 2))

coef_m1 <- broom::tidy(m1, conf.int = TRUE) %>% 
  dplyr::filter(!term == "(Intercept)") %>% 
  dplyr::mutate(pos_neg = as.factor(ifelse(estimate > 0, 2, 1))) %>% 
  dplyr::left_join(rq, by = c("term" = "variable")) %>% 
  dplyr::mutate(term_neat = c("Isolation distance", "Surrounding landmass", "Elevation", "Temperature", "Temperature variability", "Precipitation", "Precipitation variability", "Mainland plant richness", "Archipelago plant richness", "Number of islands", "Total area", "SD area", "Native rodents", "Endemic non-passerines", "Research effort", "Human arrival", "Total area:Research effort"))
```
...
# extrapolate
ar_extrap <- lapply(1:nrow(pred_trans), function(x) {
  ar <- pred_trans[x,]$Archip_10km
  
  nd <- pred_trans %>% 
    dplyr::filter(Archip_10km == ar)
  
  ext_spp_np <- nd$ext_spp_np
  
  delta_res <- nz - nd$res_eff
  
  ext_spp_np <- ext_spp_np + delta_res$res_eff * (as.vector(coef(m1)["res_eff"]) + as.vector(coef(m1)["tot_area:res_eff"]) * as.vector(coef(m1)["tot_area"]))
  
})

# extrapolate upper confidence bound
ar_extrap_upp <- lapply(1:nrow(pred_trans), function(x) {
  ar <- pred_trans[x,]$Archip_10km
  ...
  ext_spp_np <- ext_spp_np + delta_res$res_eff * (confint(m1, level = 0.99)["res_eff", 2] + confint(m1, level = 0.99)["tot_area:res_eff", 2] * confint(m1, level = 0.99)["tot_area", 2])
  
})

# extrapolate lower confidence bound
ar_extrap_low <- lapply(1:nrow(pred_trans), function(x) {
  ar <- pred_trans[x,]$Archip_10km
  ...
  ext_spp_np <- ext_spp_np + delta_res$res_eff * (confint(m1, level = 0.99)["res_eff", 1] + confint(m1, level = 0.99)["tot_area:res_eff", 1] * confint(m1, level = 0.99)["tot_area", 1])
  
  
})

ar_spp <- cbind(dplyr::select(pred, Archip_10km, ext_spp_np), ar_extrap = as.numeric(ar_extrap), ar_extrap_upp = as.numeric(ar_extrap_upp), ar_extrap_low = as.numeric(ar_extrap_low)) %>% 
  # recorded and unrecorded prehistoric extinct
  dplyr::mutate(ar_extrap = (10^ar_extrap)-1) %>%
  # CI 99%
  dplyr::mutate(ar_extrap_upp = (10^ar_extrap_upp)-1) %>%
  dplyr::mutate(ar_extrap_low = (10^ar_extrap_low)-1) %>%
  # unrecorded prehistoric extinct
  dplyr::mutate(add_spp = ar_extrap - ext_spp_np) %>% 
  # CI 99%
  dplyr::mutate(add_spp_upp = ar_extrap_upp - ext_spp_np) %>%
  dplyr::mutate(add_spp_low = ar_extrap_low - ext_spp_np)
```
